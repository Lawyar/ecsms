...
public:
  ConsumerStage(const std::string_view stageName,
                TaskRetrieveStrategy,
                std::shared_ptr<InStageConnection<In>>);

private:
  virtual void function(std::shared_ptr<In> inData) = 0;

  void function(std::shared_ptr<In> inData,
...
template <typename In>
ConsumerStage<In>::ConsumerStage(const std::string_view stageName,
              TaskRetrieveStrategy consumerStrategy,
