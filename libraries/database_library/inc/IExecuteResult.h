...
public:
  ...
  private:
    ...
    // Единственный способ получить бинарный результат - через вторую перегрузку
    // IConnection::Execute, которая в данный момент не используется и
    // перемещена в protected-секцию. Поэтому и эти методы перемещу в
    // private-секцию.
...
public:
  ...
  /// Получить название столбца (возвращает пустую строку, если передан
  /// невалидный индекс)
  virtual std::string GetColName(size_t columnIndex) const = 0;
  /// Получить индекс столбца по имени (возвращает невалидный индекс, если
  /// передано несуществующее имя) Имя (в соотвествии со стандартом SQL) по
  /// умолчанию приводится к нижнему регистру, так что вызовы
  /// GetColIndex("columnname") и GetColIndex("CoLuMnNaMe") равнозначны, кроме
  /// случая, если имя обрамлено кавычками. Таким образом, если запрос содержал
  /// явное именование столбцов с использованием кавычек (например, SELECT 1 AS
  /// "Number"), то в эту функцию надо передавать GetColIndex("\"Number\"") для
  /// определения индекса столбца, а вызовы GetColIndex("Number"), или
  /// GetColIndex("\"number\"") вернут невалидный индекс.
  virtual size_t GetColIndex(const std::string &columnName) = 0;

  /// Получить тип данных столбца (возвращает невалидный тип данных, если
  /// передан невалидный индекс)
