...
//------------------------------------------------------------------------------
/**
  \brief Интерфейс исполнителя запросов EAV.
  ...
  Метод \ref SetRegisteredEntities может создать таблицы по указанным правилам, если
  передать в него флаг \c createTables = \c true. Остальные методы не создают таблицы,
...
class IExecutorEAV {
public:
  /// Тип для названия сущности.
  /// Осмысленное название сущности.
  using EntityName = std::string;
  /// Тип для идентификатора сущности.
  /// Числовой идентификатор конкретной сущности в её таблице сущностей.
  using EntityId = int;
  ...
  /// Реестр EAV-сущностей.
  /// Одна запись в реестре хранит информацию о названии сущностей, а также о
  /// типах атрибутов, которые она использует.
  using EAVRegisterEntries = std::map<EntityName, std::vector<SQLDataType>>;
...
public
  ...
  /// \param[out] result Массив наименований атрибутов.
  ///                    В случае успеха массив прочищается перед записью
  ///                    результата.
  /// \return Статус выполнения операции
  virtual IExecuteResultStatusPtr
...
public: // Методы для вставки/обновления данных
  ...
  ///              (ISQLType::IsEmpty) переменная value, атрибут будет добавлен
  ///              в таблицу атрибутов (если его там ещё не было).
  /// \return Статус выполнения операции
...
public: // Методы для получения данных
  ...
  ///                      результат. Если будет передана непустая
  ///                      (ISQLType::IsEmpty) переменная, то вернется ошибка.
  /// \return Статус выполнения операции
