...
/**
  ...
  Таким образом, можно в вызывающем коде открыть транзакцию, вызвать один или несколько методов
  класса, после чего зафиксировать (или отменить) транзакцию, применив (или не применив) разом
  несколько изменений.

  (Если вызывающий код не открывает транзакцию, то каждый метод класса будет работать в рамках
  одной или нескольких транзакций (зависит от реализации конкретного метода))
*/
//---
class IExecutorEAV
{
...
public: // Методы для получения данных
	...
	/// Если у данной сущности в таблице атрибутов есть атрибут данного типа с данным названием,
	/// однако в таблице значений для данного идентификатора сущности и данного атрибута
	/// нет значения, то метод НЕ вернет ошибку, а вернет пустую переменную.
	/// \param entityName Название сущности
	/// \param entityId Идентфикатор сущности
	/// \param attrName Наименование атрибута
	/// \param[in,out] value Ненулевая пустая переменная, по типу которой будет определен тип атрибута,
	///                      и в которую запишется результат. Если будет передана непустая переменная,
	///                      то вернется ошибка.
	/// \return Статус выполнения операции
	...
	/// Таким образом, метод не вернет информацию о незарегистрированных типах атрибутов, если они есть.
	/// \param entityName Название сущности
