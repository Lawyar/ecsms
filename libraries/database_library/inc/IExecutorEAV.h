...
public: // Методы для вставки/обновления данных
	...
	/// а также вставить название атрибутов в таблицу атрибутов, если его там ещё не было.
	/// \param entityName Название сущности
	/// \param entityId Идентфикатор сущности. При передаче несуществующего идентификатора
	///                 вернется ошибка.
	/// \param attrName Наименование атрибута. При передаче названия несуществующего атрибута
	///                 будет добавлен новый атрибут с таким названием.
	/// \param value Значение атрибута. Это обязательно должна быть непустая (ISQLType::IsEmpty)
	///              и ненулевая переменная, в противном случае запрос не будет выполнен и
	///              вернется ошибка.
	///              (Смысл EAV в том, что мы не храним null, так что бессмысленно вставлять
	///              пустые значения, символизирующие null)
	/// \return Статус выполнения операции
	...
	/// \param entityId Идентфикатор сущности. При передаче несуществующего идентификатора
	///                 вернется ошибка.
	/// \param attrName Наименование атрибута. При передаче несуществующего названия
	///                 вернется ошибка.
	/// \param value Значение атрибута. Это обязательно должна быть ненулевая переменная,
	///              иначе запрос не будет выполнен и вернется ошибка.
	///              Если значение атрибута пустое (ISQLType::IsEmpty),
	///              то значение атрибута сущности будет удалено из таблицы значений (если оно
	///              там есть).
	///              (Смысл EAV в том, что мы не храним null, так что попытка занулить значение
	///              приводит к его удалению)
	/// \return Статус выполнения операции
	virtual IExecuteResultStatusPtr Update(const EntityName & entityName, EntityId entityId,
		const AttrName & attrName, const ValueType & value) = 0;
	/// Обновить значение для атрибута сущности в таблице значений
	/// или вставить значение атрибута в таблицу значений, если такого значения ещё не было.
	/// А также вставить название атрибута в таблицу атрибутов, если его там ещё не было.
	/// \param entityName Название сущности
	/// \param entityId Идентфикатор сущности. При передаче несуществующего идентификатора
	///                 вернется ошибка.
	/// \param attrName Наименование атрибута. При передаче названия несуществующего атрибута
	...
	///              Вне зависимости от того, пришла пустая или непустая переменная value,
	///              атрибут будет добавлен в таблицу атрибутов (если его там ещё не было).
	/// \return Статус выполнения операции
...
public: // Методы для получения данных
	...
	/// \param[out] attrValuesByType Мапа, в которой ключем является тип атрибута, а значением - 
	...
	///             В случае успеха метод очищает вектор перед вставкой.
	/// \return Статус выполнения операции
	virtual IExecuteResultStatusPtr GetAttributeValues(const EntityName & entityName,
		EntityId entityId, std::map<SQLDataType, std::vector<AttrValue>> & attrValuesByType) = 0;
};
