...
public: // Получение пустых SQL-переменных
  ...
  virtual ISQLTypeIntegerPtr
  GetSQLTypeInteger(const std::optional<StrongType<int>> &value =
  ...
  GetSQLTypeByteArray(std::vector<char> &&value) const override;
  ...
  GetSQLTypeRemoteFileId(const std::string &id) const override;
};
