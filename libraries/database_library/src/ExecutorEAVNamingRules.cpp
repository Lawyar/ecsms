...
/**
  ...
  2) <EntityName>_attribute_<AttributeType> - несколько таблиц атрибутов - по
  одной таблице для каждого типа данных, который хотим хранить. Каждая таблица
  содержит поля: "id" - идентификатор атрибута - является первичным ключом.
         "name" - название атрибута - на него наложено ограничение UNIQUE (не
  может быть разных атрибутов с одинаковыми названиями); 3)
  <EntityName>_value_<AttributeType> - таблица значений атрибутов сущностей -
  содержит поля: "entity_id" - идентификатор сущности, с которой ассоциировано
  значение в этой строке - является внешним ключом. "attribute_id" -
  идентификатор атрибута, с которым ассоциировано значение в этой строке -
  является внешним ключом. Пара (entity_id, attribute_id) является первичным
  ключом. "value" - значение атрибута для данной сущности. Это поле имеет тип
  <AttributeType>. Пример такой базы данных - имеем два типа сущностей: User и
  Product. У User есть три атрибута двух типов - имя (text), адрес (text) и дата
  рождения (timestamp). У Product есть два атрибуты двух типов - название (text)
  и цена (numeric). Тогда в базе данных будут таблицы: user,
  user_attribute_text, user_attribute_timestamp, user_value_text,
  user_value_timestamp, product, product_attribute_text,
  product_attribute_numeric, product_value_text, product_value_numeric.

  Метод RegisterEntities может создать таблицы по указанным правилам, если
...
public:
  ...
  struct EntityTable {
    static constexpr char *c_idField =
        "id"; ///< Название поля для идентификатора сущности в таблице сущностей
  };

  /// Названия полей в таблице атрибутов
  struct AttributeTable {
    static constexpr char *c_idField =
        "id"; ///< Название поля для идентификатора атрибута в таблице атрибутов
    static constexpr char *c_nameField =
        "name"; ///< Название поля для названия атрибута в таблице атрибутов
  };

  /// Название полей в таблице значений
  struct ValueTable {
    static constexpr char *c_entityIdField =
        "entity_id"; ///< Название поля для идентификатора сущности в таблице
                     ///< значений
    static constexpr char *c_attributeIdField =
        "attribute_id"; ///< Название поля для идентификатора атрибута в таблице
                        ///< значений
    static constexpr char *c_valueField =
        "value"; ///< Название поля для значения в таблице значений
  };
...
public:
  ...
  RegisterEntities(const EAVRegisterEntries &entries, bool createTables) = 0;
  ...
  virtual std::string
  GetEntityTable_Full_IdField(const std::string &entityName) const = 0;
  ...
  virtual std::string GetValueTable_Full_AttributeIdField(
...
public: // Методы для получения данных
  ...
  /// Переменная value - должна быть пустой (сконструированной конструктором по
  /// умолчанию) переменной соответствующего типа. Например, если мы хотим
  /// получить значение текстового аттрибута, то нужно передать ненулевой
  /// указатель ISQLTypeTextPtr. Результат метода запишется в value.
  ...
  virtual IExecuteResultStatusPtr
  GetAttributeValues(const EntityName &entityName, EntityId entityId,
                     std::vector<AttrValue> &attrValues) = 0;
};
