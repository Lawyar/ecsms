...
private:
  const ExternalInfo m_externalInfo; ///< Внешняя информация об ошибке на момент
                                     ///< создания объекта
  const InternalInfo
      m_internalInfo; ///< Внутренняя информация об ошибке на момент создания
                      ///< объекта
                      ///  (должен идти ниже m_externalInfo, поскольку при
                      ///  инициализации использует его поле)
...
private:
  ...
  static PGExecuteResultStatus::InternalInfo
  createInternalInfo(ExecStatusType pqStatus);
};
