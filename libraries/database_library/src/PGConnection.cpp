...
static int ResultFormatToPQ(ResultFormat type) {
  ...
  case ResultFormat::Invalid:
    result = text;
    fprintf(stderr, "[Warning] Passed ResultFormat::Invalid. It is treated as "
                    "ResultFormat::Text.\n");
    break;
  default:
    fprintf(stderr, "[Error] Passed unknown ResultFormat value. It is treated "
                    "as ResultFormat::Text.\n");
    result = text;
    break;
  }
  return result;
...
IFilePtr PGConnection::CreateRemoteFile() {
  ...
  return oid == InvalidOid
             ? nullptr
             : std::make_shared<PGRemoteFile>(weak_from_this(), oid);
}
...
/**
  ...
  \return Если передано имя файла, которое не соответствует правилам именования,
  то возвращает nullptr. В противном случае функция возвращает ненулевой
  указатель. При этом полученный файл может не существовать - это проверяется на
...
IExecuteResultPtr PGConnection::Execute(const std::string &singleCommand,
  ...
  for (auto &&arg : args) {
    std::visit(
        [&pqParamValues, &pqParamLengths, &pqParamFormats](auto &&arg) {
  ...
  std::lock_guard guard(m_mutex);
  PGresult *pqResult =
      PQexecParams(m_conn, singleCommand.c_str(), args.size(), pqTypes.data(),
                   pqParamValues.data(), pqParamLengths.data(),
                   pqParamFormats.data(), pqResultFormat);
...
std::optional<Oid> PGConnection::fileNameToOid(const std::string &filename) {
  auto number = utils::string::StringToNumber(
      static_cast<long (*)(const std::string &, size_t *, int)>(&std::stol),
      filename);
  if (!number || *number < std::numeric_limits<Oid>::min() ||
      *number > std::numeric_limits<Oid>::max())
    return std::nullopt;
