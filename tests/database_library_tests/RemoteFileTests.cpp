...
static const int _InitializeSeed =
    (srand(time(nullptr)), 0); ///< Инициализация семени рандома
...
/// Режим открытия на запись прочищает файл, созданный в рамках той же
/// транзакции
TEST_F(TestWithValidRemoteFile,
       WriteModeClearsFileCreatedByTheSameTransaction) {
...
TEST_F(TestWithValidRemoteFile,
       WriteModeClearsFileCreatedInPreviousTransaction) {
  constexpr size_t size = 1000;
...
/// Режим открытия на дозапись дописывает файл, созданный в рамках той же
/// транзакции
TEST_F(TestWithValidRemoteFile,
       AppendModeAppendsFileCreatedByTheSameTransaction) {
  constexpr size_t size = 1000;
...
/// Режим открытия на дозапись дописывает файл, созданный в предыдущей
/// транзакции
TEST_F(TestWithValidRemoteFile,
       AppendModeAppendsFileCreatedInPreviousTransaction) {
  constexpr size_t size = 1000;
  std::vector<char> bytes1;
  std::vector<char> bytes2;
...
/// Можно дописать данные в файл в рамках одного сеанса работы с файлом в рамках
/// одной транзакции в режиме открытия на запись
TEST_F(TestWithValidRemoteFile,
       CanAddDataToFileInOneSessionInOneTransactionWithWriteMode) {
  constexpr size_t size = 1000;
...
/// Можно дописать данные в файл в рамках одного сеанса работы с файлом в рамках
/// одной транзакции в режиме открытия на дозапись
TEST_F(TestWithValidRemoteFile,
       CanAddDataToFileInOneSessionInOneTransactionWithAppendMode) {
  constexpr size_t size = 1000;
...
static std::vector<char> FastGenerateBytes(size_t count,
  ...
  std::vector<char> arr(count % sizeof(int) == 0
                            ? count
...
TEST_F(TestWithValidRemoteFile, CanWriteAndReadLargeData) {
  ...
  clock_t generateClocks =
      0; ///< Время, потраченное на генерацию случайных чисел
  ...
  FastGenerateBytes(
      0,
      &seed); // зададим то же семя рандома, чтобы генерировать такие же массивы
  ...
  std::cout << "Time spent generating random numbers: "
            << (float)generateClocks / CLOCKS_PER_SEC << " seconds"
            << std::endl;
  std::cout << "Time spent writing data: "
            << (float)writeClocks / CLOCKS_PER_SEC << " seconds" << std::endl;
  std::cout << "Time spent reading data: " << (float)readClocks / CLOCKS_PER_SEC
            << " seconds" << std::endl;
...
/// Записанные в пустой файл данные не сохраняются при обрыве транзакции в
/// режиме открытия на запись
TEST(RemoteFile,
     DataWrittenToEmptyFileIsNotSavedWhenTransactionAbortsInOpenWriteMode) {
  std::string filename;
  {
    auto &&connection =
        GetDatabaseManager().GetConnection(c_PostgreSQLConnectionURL);
...
/// Записанные в пустой файл данные не сохраняются при откате транзакции в
/// режиме открытия на запись
TEST(RemoteFile,
     DataWrittenToEmptyFileIsNotSavedWhenTransactionRollbacksInOpenWriteMode) {
  std::string filename;
  {
    auto &&connection =
        GetDatabaseManager().GetConnection(c_PostgreSQLConnectionURL);
...
/// Записанные в пустой файл данные не сохраняются при обрыве транзакции в
/// режиме открытия на дозапись
TEST(RemoteFile,
     DataWrittenToEmptyFileIsNotSavedWhenTransactionAbortsInOpenAppendMode) {
  std::string filename;
  {
    auto &&connection =
        GetDatabaseManager().GetConnection(c_PostgreSQLConnectionURL);
...
/// режиме открытия на дозапись
TEST(RemoteFile,
     DataWrittenToEmptyFileIsNotSavedWhenTransactionRollbacksInOpenAppendMode) {
  std::string filename;
  {
    auto &&connection =
        GetDatabaseManager().GetConnection(c_PostgreSQLConnectionURL);
...
/// Записанные в непустой файл данные не сохраняются при обрыве транзакции в
/// режиме открытия на запись
TEST(RemoteFile,
     DataWrittenToNonEmptyFileIsNotSavedWhenTransactionAbortsInOpenWriteMode) {
  std::string filename;
...
/// Записанные в непустой файл данные не сохраняются при откате транзакции в
/// режиме открытия на запись
TEST(
    RemoteFile,
    DataWrittenToNonEmptyFileIsNotSavedWhenTransactionRollbacksInOpenWriteMode) {
  std::string filename;
  const std::vector<char> writtenBytes = GenerateBytes(1000);
  {
    // Сначала создадим непустой файл
...
/// режиме открытия на дозапись
TEST(RemoteFile,
     DataWrittenToNonEmptyFileIsNotSavedWhenTransactionAbortsInOpenAppendMode) {
  std::string filename;
  const std::vector<char> writtenBytes = GenerateBytes(1000);
  {
    // Сначала создадим непустой файл
...
TEST(
    RemoteFile,
    DataWrittenToNonEmptyFileIsNotSavedWhenTransactionRollbacksInOpenAppendMode) {
  std::string filename;
  const std::vector<char> writtenBytes = GenerateBytes(1000);
  {
    // Сначала создадим непустой файл
