...
TEST(Connection, ConnectionWithValidURLIsValid) {
  ...
  IConnectionPtr connection =
      databaseManager.GetConnection(c_PostgreSQLConnectionURL);
...
TEST(Connection, InvalidConnectionExecuteReturnsInvalidResult) {
  ...
  auto &&result = connection->Execute(
      ...
      "InvalidConnectionExecuteReturnsInvalidResult(id integer PRIMARY KEY);");
...
TEST(Connection, ValidConnectionExecuteReturnsValidResult) {
  ...
  // Создадим таблицу
  {
    auto &&result = connection->Execute(
        "CREATE TABLE IF NOT EXISTS "
        "ValidConnectionExecuteReturnsValidResult(id integer PRIMARY KEY);");
    ASSERT_NE(result, nullptr);
...
TEST(Connection, InvalidQueryReturnsInvalidResult) {
  ...
  // Создадим таблицу
  {
    ...
    auto &&result = connection->Execute(
        "CREATE TABLE IF NOT EXIST InvalidQueryReturnsInvalidResult(id integer "
        "PRIMARY KEY);");
    ASSERT_NE(result, nullptr);
    ...
    ASSERT_EQ(status->GetErrorMessage(),
              ...
              "LINE 1: CREATE TABLE IF NOT EXIST "
              "InvalidQueryReturnsInvalidResult(i...\n"
              "                            ^\n");
  }
}
...
TEST(Connection, ValidQueryReturnsValidResult) {
  ...
  {
    auto &&result =
        connection->Execute("CREATE TABLE ValidQueryReturnsValidResult(id "
                            "integer PRIMARY KEY, data text);");
    ASSERT_NE(result, nullptr);

    auto &&status = result->GetCurrentExecuteStatus();
    ASSERT_TRUE(status && !status->HasError() &&
                status->GetStatus() == ResultStatus::OkWithoutData);
  }
  ...
  // Вставим в эту таблицу одну строку
  {
    auto &&result = connection->Execute(utils::string::Format(
        "INSERT INTO ValidQueryReturnsValidResult VALUES({}, '{}');", id,
        data));
    ASSERT_NE(result, nullptr);
  ...
  {
    auto &&result =
        connection->Execute("SELECT * FROM ValidQueryReturnsValidResult;");
    ASSERT_NE(result, nullptr);

    auto &&status = result->GetCurrentExecuteStatus();
    ASSERT_TRUE(status && !status->HasError() &&
                status->GetStatus() == ResultStatus::OkWithData);
  ...
  {
    auto &&result =
        connection->Execute("DROP TABLE ValidQueryReturnsValidResult;");
    ASSERT_NE(result, nullptr);
...
TEST(Connection, JustBeginTransaction) {
...

  {
    ...
    connection->Execute(
        "CREATE TABLE JustBeginTransaction(id INTEGER PRIMARY KEY);");
...
TEST(Connection, BeginCommitTransaction) {
...

  {
    ...
    connection->Execute(
        "CREATE TABLE BeginCommitTransaction(id INTEGER PRIMARY KEY);");
...
TEST(Connection, BeginRollbackTransaction) {
...

  {
    ...
    connection->Execute(
        "CREATE TABLE BeginRollbackTransaction(id INTEGER PRIMARY KEY);");
...
TEST(Connection, CanGetColType) {
  ...
  // Порядок (типов) аргументов в запросе должен соответствовать порядку типов в
  // SQLDataType
  const std::string query =
      "SELECT 1, 'text', '\\x0123456789ABCDEF'::bytea, 20000::oid, 1.5;";
...
TEST(Connection, CanGetColName) {
  ...
  const std::string query =
      "SELECT 1 as name, 2 as name, 'text' as \"Name 2\", 3;";
  auto &&result = connection->Execute(query);
  ASSERT_NE(result, nullptr);
  auto &&status = result->GetCurrentExecuteStatus();
  ASSERT_TRUE(status && !status->HasError());
...
TEST(Connection, CanGetValue) {
  ...
  {
    auto &&result =
        connection->Execute("SELECT 1, 'text', '\\xABCD'::bytea, NULL;");
    ASSERT_NE(result, nullptr);
    auto &&status = result->GetCurrentExecuteStatus();
    ASSERT_TRUE(status && !status->HasError());
...
TEST(Connection, CommandsAreExecutedTransactionally) {
...

  {
    auto &&result = connection->Execute(
        ...
        "CREATE TABLE CommandsAreExecutedTransactionally(id INTEGER PRIMARY "
        "KEY);\n"
...

  {
    ...
    // Если бы команды выполнялись не транзакционно, то таблица бы создалась на
    // предыдущем вызове. И здесь мы бы получили корректный результат, а не
    // ошибку.
    ASSERT_TRUE(status->HasError());
  }
}
...
/// По валидному соединению можно получить существующий удаленный файл, и он
/// доступен
...
/// По валидному соединению можно получить несуществующий удаленный файл, но он
/// будет недоступен
TEST(Connection, CanGetNonExistingRemoteFileWithValidConnection) {
  {
    ...
    // Проверка на недоступность (раз файл не существует, то не должен
    // открываться)
