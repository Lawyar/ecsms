...
TEST(SQLTypeByteArray, GetSetValue) {
  ...
  std::vector<std::tuple<ISQLTypeByteArrayPtr, // SQL-переменная
                         std::optional<std::vector<char>>, // Её ожидаемое
                                                           // значение
                         std::vector<char>, // Аргумент для SetValue
                         std::optional<std::vector<char>> // Ожидаемое значение
                                                          // после SetValue
                         >>
      testData = {
          // Конструктор с валидными данными и SetValue с валидными данными
          {converter->GetSQLTypeByteArray(), std::nullopt,
           std::vector<char>{1, 2, 3}, std::vector<char>{1, 2, 3}},
          {converter->GetSQLTypeByteArray(), std::nullopt, std::vector<char>{},
           std::vector<char>{}},
          {converter->GetSQLTypeByteArray({}), std::vector<char>{},
           std::vector<char>{}, std::vector<char>{}},
          {converter->GetSQLTypeByteArray({1, 2, 3}),
           std::vector<char>{1, 2, 3}, std::vector<char>{},
           std::vector<char>{}},
          {converter->GetSQLTypeByteArray({1, 2, 3}),
           std::vector<char>{1, 2, 3}, std::vector<char>{1, 2, 3},
           std::vector<char>{1, 2, 3}},
          {converter->GetSQLTypeByteArray({1, 2, 3}),
           std::vector<char>{1, 2, 3}, std::vector<char>{4, 5, 6},
           std::vector<char>{4, 5, 6}},
          {converter->GetSQLTypeByteArray({1, 2, 3, 4, 0}),
           std::vector<char>{1, 2, 3, 4, 0}, std::vector<char>{1, 2, 3},
           std::vector<char>{1, 2, 3}},
      };
...
TEST(SQLTypeByteArray, GetType) {
  auto &&converter = GetDatabaseManager().GetSQLTypeConverter();
  for (auto &&sqlVar :
       {converter->GetSQLTypeByteArray(), converter->GetSQLTypeByteArray({}),
        converter->GetSQLTypeByteArray({0}),
        converter->GetSQLTypeByteArray({1, 2, 3})}) {
...
TEST(SQLTypeByteArray, ToSQLString) {
  ...
  for (int i = std::numeric_limits<char>::min();
    ...
    testData.push_back({converter->GetSQLTypeByteArray({currentChar}),
                        "'\\x" + CharToHex(currentChar) + "'::BYTEA"});
  }
...
TEST(SQLTypeByteArray, ReadFromSQL) {
  ...
  std::vector<std::tuple<ISQLTypeByteArrayPtr, // SQL-переменная
      ...
      testData = {
          ...
          {converter->GetSQLTypeByteArray(), "\\xAB", true,
           std::vector<char>{-85}},
          {converter->GetSQLTypeByteArray({}), "\\xab", true,
           std::vector<char>{-85}},
          {converter->GetSQLTypeByteArray({1, 2, 3}), "\\x0123456789ABCDEF00",
           true, std::vector<char>{1, 35, 69, 103, -119, -85, -51, -17, 0}},
          {converter->GetSQLTypeByteArray({0, -1, -2}), "\\x0123456789abcdef",
           true, std::vector<char>{1, 35, 69, 103, -119, -85, -51, -17}},
          ...
          {converter->GetSQLTypeByteArray({1, 2, 3}), "\\x10 ", false,
           std::nullopt},
          {converter->GetSQLTypeByteArray({1, 2, 3}), "", false, std::nullopt},
          {converter->GetSQLTypeByteArray({0, -1, -2}), " \\x10 ", false,
           std::nullopt}};
