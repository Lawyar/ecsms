...
TEST(SQLTypeRemoteFileId, ReadFromSQL) {
  ...
  const std::vector<std::tuple<std::string, // Строка для чтения
                               ...
                               std::optional<std::string> // Ожидаемое значение
                                                          // после чтения
                               >>
      testData{{"", false, std::nullopt},
               {"0", true, "0"},
               {"12345", true, "12345"},
  ...
  for (auto &&[sqlString, expectedReadResult, expectedGetValueResult] :
    ...
    auto sqlVars = {converter->GetSQLTypeRemoteFileId(),
                    ...
                    converter->GetSQLTypeRemoteFileId("0"),
                    converter->GetSQLTypeRemoteFileId("12345"),
                    converter->GetSQLTypeRemoteFileId("hello"),
