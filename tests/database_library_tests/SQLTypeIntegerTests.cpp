	...
	for (auto && [sqlString, expectedReadResult, expectedGetValueResult] : testData)
	{
		...
		for (auto && sqlVar : sqlVars)
		{
			...
			if (result)
				...
				// То есть её заполненность не должна была поменяться
				// (если была пустой - должна остаться пустой,
				//  если была заполненной - должна остаться заполненной)
				ASSERT_EQ(sqlString.empty(), sqlStringCopy.empty());
		}
	}
}
