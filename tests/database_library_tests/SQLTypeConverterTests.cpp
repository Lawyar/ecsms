...
TEST(SQLTypeConverter, CanGetSQLVariable) {
  ...
  ASSERT_TRUE(std::dynamic_pointer_cast<ISQLTypeInteger>(
      converter->GetSQLVariable(SQLDataType::Integer)));
  ASSERT_TRUE(std::dynamic_pointer_cast<ISQLTypeText>(
      converter->GetSQLVariable(SQLDataType::Text)));
  ASSERT_TRUE(std::dynamic_pointer_cast<ISQLTypeByteArray>(
      converter->GetSQLVariable(SQLDataType::ByteArray)));
}
...
TEST(SQLTypeConverter, CanGetSQLTypeByteArray) {
  ...
  auto sqlVar2 =
      converter->GetSQLTypeByteArray(std::vector<char>({1, 2, 3, 4, 5}));
