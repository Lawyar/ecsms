...
void openSavingMessageBox(SaveFunc save_func,
                          DontSaveFunc dont_save_func,
...
inline QString getSaveFileName(QWidget* parent,
  ...
  auto&& file_name = QFileDialog::getSaveFileName(parent, "Сохранение", filter,
                                                  selected_filter);
...
inline QString getOpenFileName(QWidget* parent,
                               ...
                               const QString& selected_filter) {
  auto&& file_name =
      QFileDialog::getOpenFileName(parent, "Открытие", filter, selected_filter);
  if (file_name.size() == 0) {
    QMessageBox::warning(parent, "Внимание", "Файл не был выбран");
    file_name.clear();
    return file_name;
...
inline bool getXMLFromFile(QWidget* parent,
...
inline bool saveXMLToFile(QWidget* parent,
                          const QString& file_name,
                          const QStandardItemModel* model) {
  QFile file(file_name);
...
inline bool getYAMLFromFile(QWidget* parent,
                            const QString& file_name,
                            FieldModel& f,
                            VisualizationModel& v,
                            int& name_maker_counter) {
  QFile file(file_name);
  if (!file.open(QIODevice::ReadOnly)) {
    QMessageBox::critical(parent, "Ошибка",
                          "Не удалось открыть файл " + file_name);
  ...
  auto&& s = file.readAll().toStdString();
  YAML::Node main_node = YAML::Load(s);
  f = main_node["field_model"].as<FieldModel>();
  name_maker_counter = main_node["name_maker_counter"].as<int>();
  v = main_node["visualization_model"].as<VisualizationModel>();

  file.close();
  return true;
}

inline bool saveYAMLToFile(QWidget* parent,
                           const QString& file_name,
                           const FieldModel& f,
                           const VisualizationModel& v,
                           int name_maker_counter) {
  QFile file(file_name);
  if (!file.open(QIODevice::WriteOnly)) {
    QMessageBox::critical(parent, "Ошибка",
  ...
  YAML::Node main_node;
  main_node["field_model"] = f;
  main_node["name_maker_counter"] = name_maker_counter;
  main_node["visualization_model"] = v;

  std::ostringstream s;
  s << main_node;
  file.write(s.str().c_str());
  file.close();
  QMessageBox::information(parent, "Сохранение",
                           "Файл успешно сохранён в " + file_name);
  return true;
